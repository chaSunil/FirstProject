...
public class JSONParseAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * String json_str = ""; JSONObject json = new
		 * JSONObject("{\"name\":\"일길동\",\"age\":30,\"hobby\":[\"독서\",\"낚시\"]};");
		 * 
		 * String name = json.getString("name"); int age = json.getInt("age");
		 * System.out.printf("이름:%s 나이:%d\n", name, age);
		 * 
		 * // JSON 배열 읽어오기 JSONArray hobbyArray = json.getJSONArray("hobby"); for(int
		 * i=0; i<hobbyArray.length(); i++) { // i=0, i=1 String hobby =
		 * hobbyArray.getString(i); System.out.printf("취미 %d : %s\n", i+1, hobby); }
		 */
